...
class DataLoggerComponent extends React.Component {
  ...
  render() {
    let log = (this.props.inputData || []).map((data, index) => {
      return <div key={'log_' + index.toString()}>Longitude: {data.longitude}, Latitude: {data.latitude}, Drone Detected: {data.isDrone ? 'YES' : 'NO'}</div>
    });
    if (log.length > 200) {
      log = log.slice(-200);
    }
